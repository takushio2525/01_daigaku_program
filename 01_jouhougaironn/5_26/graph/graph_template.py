...
class Graph:
    def __init__(self, subplot_index=None, font_path='/System/Library/Fonts/ヒラギノ角ゴシック W4.ttc'):
...
class GraphWindow:
    def __init__(self, n_subplots=None, font_path='/System/Library/Fonts/ヒラギノ角ゴシック W4.ttc', figsize=None):
        self.font_path = font_path
        self.graphs = []
        self.n_subplots = n_subplots  # Noneなら自動決定
        self.figsize = figsize  # Noneなら自動決定
    ...
    def show(self):
        n = self.n_subplots if self.n_subplots is not None else len(
            self.graphs)
        # figsize自動調整: 未指定なら(8, 4*n)
        figsize = self.figsize if self.figsize is not None else (8, 4 * n)
        fig, axs = plt.subplots(n, 1, figsize=figsize)
        if n == 1:
            axs = [axs]
        colors = ["blue", "red", "green", "orange", "purple"]
        for graph in self.graphs:
            ax = axs[graph.subplot_index]
