...
corr_sales_service = data['売上高 (億円)'].corr(data['1. 接客スコア'])
corr_sales_assortment = data['売上高 (億円)'].corr(data['2. 品揃えスコア'])
corr_sales_location = data['売上高 (億円)'].corr(data['3. 立地スコア'])


print(f"売上高と接客スコアの相関係数: {corr_sales_service}")
print(f"売上高と品揃えスコアの相関係数: {corr_sales_assortment}")
print(f"売上高と立地スコアの相関係数: {corr_sales_location}")
# # g1を個別に保存する例
